...
func _ready():
	...
	# Scale the laser's fixed thickness to match world-scale (its length is
	# already rescaled dynamically elsewhere, but its cross-section is not)
	$Laser.mesh.size.x *= _world_scale
	$Laser.mesh.size.y *= _world_scale
	$Target.mesh.radius *= _world_scale
	$Target.mesh.height *= _world_scale
